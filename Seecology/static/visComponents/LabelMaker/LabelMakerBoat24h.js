...
function LabelMakerBoat24h(){
	...
	* ########################################################################################################################################*/
	object.render = function(){
		...
		audioSeconds = Math.round((parseDate(toData) - parseDate(fromData))/60/1000);
		
		//Correção do arquivo com menos de 24h
		if (audioSeconds<1400){
			audioSeconds = audioSeconds - 60;
		}
		
		samples = audioSeconds/sampleWindow;
		console.log("seconds", audioSeconds);
		...
		xScale_sec = d3.scaleTime()
		  	.domain([parseDate(fromData), parseDate(toData)])
			//.domain([0, 60])
			.rangeRound([0, width]);
		...
		//TODO: colocar data inicial e final aqui
...
//		handle = gBrushes.selectAll(".handle--e")
//			.attr("display", "none");

		//gBrushes.selectAll(".handle--e").attr("fill", "none");
...
//		    .classed("tick--minor");
		...
		//Time axis
		//TODO: Definir o número de ticks e time format
		plotArea.append('g')
		...
		//Label the sample
		if( currentSample < samples ){ //currentSample > 0 && 
...
//	    var labelName = document.getElementById('addlabelname').value.trim();
//	    var dbname = document.getElementById('dbName').value;
//	    var dirname = document.getElementById('dirName').value;
//	    var feature = document.getElementById('lcfeatures').value;
...
//	    var select = document.getElementById("labelslist");
	    ...
	    for(id in mySelections) {
	        ...
	        console.log("start", start, mySelections[id].start);
	        console.log("end", end, mySelections[id].end);
	        
	        for (i=start; i<end; i+=parseInt(sampleWindow)) {
	        	samplesText.push(Math.round(i/sampleWindow));
		    }	        
	    }

	    //console.log(samplesText);	    
	    fillSelection(samplesText);
...
//	    document.getElementById('dirName1').value = document.getElementById('dirName').value;
	    ...
	    //TODO: Criar neste ponto um form sem reenvio da página. submeter para uma página PHP que irá
        // 		gerar um arquivo originalLabelsInput.csv no diretório destino (nome aleatório).
        //		Após criar o arquivo devo enviar o comando submit do formulário "change" para que o sistema
        //		use os novos labels
...
//		});
	...
	object.fromData = function(value){
		if (!arguments.length) return fromData;
		fromData = value;
		return object;
	};
	
	object.toData = function(value){
		if (!arguments.length) return toData;
		toData = value;
		return object;
	};
